...
def test_detect_web_servers():
    ...
    assert any(t.name == "nginx" and t.version == "1.24.0" and t.category == "server" for t in techs)
...
def test_detect_modsecurity():
    resp = Response(
        status_code=403,
        headers=[NameValue(name="Server", value="Apache/2.4.41 (Ubuntu) mod_security/2.9.3")],
