...
SERVER_RULES: list[tuple[str, str, re.Pattern]] = [
    ...
    ("Microsoft-IIS", "server", re.compile(r"Microsoft-IIS(?:/([\d.]+))?", re.IGNORECASE)),
    ...
    ("Apache Tomcat", "server", re.compile(r"(?:Apache-Coyote|Tomcat)(?:/([\d.]+))?", re.IGNORECASE)),
...
HEADER_RULES: list[tuple[str, str, str, re.Pattern]] = [
    ("x-powered-by", "PHP", "language", re.compile(r"PHP(?:/([\d.]+))?", re.IGNORECASE)),
    ...
    ("x-powered-by", "Servlet", "language", re.compile(r"Servlet(?:/([\d.]+))?", re.IGNORECASE)),
    ("x-powered-by", "Phusion Passenger", "server", re.compile(r"Phusion Passenger(?:/([\d.]+))?", re.IGNORECASE)),
    ...
    ("x-generator", "WordPress", "cms", re.compile(r"WordPress\s*([\d.]*)", re.IGNORECASE)),
...
COOKIE_RULES: list[tuple[str, str, str, re.Pattern]] = [
    ...
    ("ASP.NET_SessionId", "ASP.NET", "framework", re.compile(r"^(?:ASP\.NET_SessionId|ASPSESSIONID\w+)$", re.IGNORECASE)),
    (".AspNetCore.Session", "ASP.NET Core", "framework", re.compile(r"^\.AspNetCore\.", re.IGNORECASE)),
    ("csrftoken", "Django", "framework", re.compile(r"^csrftoken$", re.IGNORECASE)),
    ("laravel_session", "Laravel", "framework", re.compile(r"^laravel_session$", re.IGNORECASE)),
    ("XSRF-TOKEN", "Laravel / Angular", "framework", re.compile(r"^XSRF-TOKEN$", re.IGNORECASE)),
    ("connect.sid", "Express / Node.js", "framework", re.compile(r"^connect\.sid$", re.IGNORECASE)),
    ("rack.session", "Ruby on Rails", "framework", re.compile(r"^(?:rack\.session|_rails_session)$", re.IGNORECASE)),
    ("wordpress_logged_in", "WordPress", "cms", re.compile(r"^wordpress(?:_logged_in|_sec|_test_cookie)", re.IGNORECASE)),
    ("wp-settings", "WordPress", "cms", re.compile(r"^wp-settings-", re.IGNORECASE)),
    ("ci_session", "CodeIgniter", "framework", re.compile(r"^ci_session$", re.IGNORECASE)),
    ("cakephp", "CakePHP", "framework", re.compile(r"^cakephp", re.IGNORECASE)),
    ("sf_redirect", "Symfony", "framework", re.compile(r"^sf_redirect$", re.IGNORECASE)),
...
BODY_RULES: list[tuple[str, str, re.Pattern, str | None]] = [
    ("ASP.NET", "framework", re.compile(r"id=[\"']__VIEWSTATE[\"']|name=[\"']__VIEWSTATE[\"']", re.IGNORECASE), None),
    ("ASP.NET", "framework", re.compile(r"id=[\"']__EVENTVALIDATION[\"']", re.IGNORECASE), None),
    ("WordPress", "cms", re.compile(r"/wp-(?:content|includes|json)/", re.IGNORECASE), None),
    ("WordPress", "cms", re.compile(r"<meta\s+name=[\"']generator[\"']\s+content=[\"']WordPress\s*([\d.]*)[\"']", re.IGNORECASE), "\\1"),
    ("Drupal", "cms", re.compile(r"<meta\s+name=[\"']generator[\"']\s+content=[\"']Drupal\s*([\d.]*)[\"']", re.IGNORECASE), "\\1"),
    ("Drupal", "cms", re.compile(r"sites/default/files/|Drupal\.settings", re.IGNORECASE), None),
    ("Joomla", "cms", re.compile(r"<meta\s+name=[\"']generator[\"']\s+content=[\"']Joomla!?\s*([\d.]*)[\"']", re.IGNORECASE), "\\1"),
    ("Ghost", "cms", re.compile(r"<meta\s+name=[\"']generator[\"']\s+content=[\"']Ghost\s*([\d.]*)[\"']", re.IGNORECASE), "\\1"),
    ("Hugo", "cms", re.compile(r"<meta\s+name=[\"']generator[\"']\s+content=[\"']Hugo\s*([\d.]*)[\"']", re.IGNORECASE), "\\1"),
    ("Gatsby", "frontend", re.compile(r"<meta\s+name=[\"']generator[\"']\s+content=[\"']Gatsby\s*([\d.]*)[\"']", re.IGNORECASE), "\\1"),
    ("Next.js", "framework", re.compile(r"id=[\"'](?:__NEXT_DATA__|__next)[\"']|/_next/static/", re.IGNORECASE), None),
    ("Nuxt.js", "framework", re.compile(r"id=[\"']__NUXT__[\"']|/_nuxt/", re.IGNORECASE), None),
    ("React", "frontend", re.compile(r"data-reactroot|react\.production\.min\.js|react\.development\.js", re.IGNORECASE), None),
    ("Vue.js", "frontend", re.compile(r"data-v-[a-f0-9]+|vue(?:\.runtime)?(?:\.min)?\.js", re.IGNORECASE), None),
    ("Angular", "frontend", re.compile(r"ng-version=[\"']([^\"']+)[\"']|ng-app=[\"']", re.IGNORECASE), "\\1"),
    ("jQuery", "frontend", re.compile(r"jquery(?:-([\d.]+))?(?:\.min)?\.js|jQuery\s*v([\d.]+)", re.IGNORECASE), "\\1"),
    ("Bootstrap", "frontend", re.compile(r"bootstrap(?:-([\d.]+))?(?:\.min)?\.(?:css|js)", re.IGNORECASE), "\\1"),
    ("Tailwind CSS", "frontend", re.compile(r"tailwindcss(?:\.min)?\.css|class=[\"'][^\"']*(?:grid-cols-|flex-col|space-y-|bg-opacity-)[^\"']*[\"']", re.IGNORECASE), None),
    ("HTMX", "frontend", re.compile(r"htmx(?:\.min)?\.js|hx-get=|hx-post=|hx-target=", re.IGNORECASE), None),
    ("Alpine.js", "frontend", re.compile(r"alpine(?:\.min)?\.js|x-data=|x-bind=|x-show=", re.IGNORECASE), None),
    ("Livewire", "frontend", re.compile(r"wire:initial-data|wire:id=", re.IGNORECASE), None),
    ("Django", "framework", re.compile(r"name=[\"']csrfmiddlewaretoken[\"']|django-admin", re.IGNORECASE), None),
    ("Spring Boot", "framework", re.compile(r"Whitelabel Error Page", re.IGNORECASE), None),
    ("Swagger / OpenAPI", "documentation", re.compile(r"swagger-ui(?:-bundle)?\.js|id=[\"']swagger-ui[\"']", re.IGNORECASE), None),
]


def detect_technologies(response: Response, request: Request | None = None) -> list[TechDetection]:
    """Inspect response headers, cookies, and body to identify web technologies."""
    detected: dict[str, TechDetection] = {}

    def _add(name: str, category: str, version: str | None = None, confidence: str = "high", evidence: str = "") -> None:
        key = name.lower()
        if key in detected:
            cur = detected[key]
            ...
            if evidence and evidence not in cur.evidence:
                cur.evidence = f"{cur.evidence}, {evidence}" if cur.evidence else evidence
            return
        ...
        if h.name.lower() == "server" and h.value:
            for s_name, cat, pat in SERVER_RULES:
                ...
                if m:
                    ...
                    _add(s_name, cat, version=ver, confidence="high", evidence=f"Server: {h.value}")
    ...
    for h in response.headers:
        ...
        for target_hdr, tech_name, cat, pat in HEADER_RULES:
            if h_name_lower == target_hdr and h.value:
                ...
                if m:
                    ver = m.group(1) if m.groups() and m.group(1) else None
                    _add(tech_name, cat, version=ver, confidence="high", evidence=f"{h.name}: {h.value}")
    ...
    if body:
        for tech_name, cat, pat, ver_group in BODY_RULES:
            ...
            if m:
                ...
                _add(tech_name, cat, version=ver, confidence="high", evidence=f"Body: {snippet}")
    ...
    if target_url:
        ...
        if path.endswith((".aspx", ".ashx", ".asmx")):
            _add("ASP.NET", "framework", confidence="high", evidence=f"URL path: {path}")
