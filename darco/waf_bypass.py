...
_VENDOR_TECHNIQUES: dict[str, list[str]] = {
    "Cloudflare": ["host_swap", "path_normalize", "header_case", "http1_0", "x_original_url", "encoding"],
    "AWS WAF / CloudFront": ["host_swap", "header_case", "encoding", "content_type"],
    "Akamai Kona Site Defender": ["header_case", "path_normalize", "x_original_url", "encoding"],
    ...
    "Microsoft Azure Front Door / AppGW WAF": ["header_case", "encoding", "content_type"],
...
_GENERIC_TECHNIQUES = [
    "header_case", "path_normalize", "encoding", "http1_0",
    "x_original_url", "content_type", "cookie_pad",
]
...
def _build_techniques(target_waf: str | None, origin_ip: str | None = None) -> list[BypassTechnique]:
    ...
    if "header_case" in wanted:
        techs.append(BypassTechnique(
            ...
            headers=[
                ...
                NameValue(name=_rand_case("User-Agent"),
    ...
    if "path_normalize" in wanted:
        techs.append(BypassTechnique(
            ...
            headers=[],
            transforms=["inject /./ and // into request path", "try %2e%2e traversal padding"],
    ...
    if "encoding" in wanted:
        techs.append(BypassTechnique(
            ...
            headers=[NameValue(name="Content-Type",
                               value="application/x-www-form-urlencoded; charset=UTF-8")],
            transforms=["double URL-encode parameter values", "use overlong UTF-8 for quotes"],
...
def apply_bypass(req: Request, technique_ids: list[str] | None = None,
                 origin_ip: str | None = None) -> tuple[Request, list[str]]:
    ...
    applied: list[str] = []
    ids = technique_ids or ["header_case", "path_normalize", "encoding", "x_original_url"]
    ...
    for tid in ids:
        if tid == "header_case":
            ...
            for h in req.headers:
                if h.name.lower() == "content-type" and "charset" not in h.value.lower():
            ...
            if "://" in req.url:
                ...
                if pathpart:
                    newpath = pathpart.replace("/", "/./", 1) if "/" in pathpart else "/" + pathpart
                    req.url = f"{scheme}://{hostpart}/{newpath}"
            applied.append("path_normalize")
        elif tid == "http1_0":
            applied.append("http1_0")  # handled at transport layer (not in Request model)
